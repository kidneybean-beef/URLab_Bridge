...
from urlab_policy.go2.twist_commands import (  # noqa: E402
    KeyboardCommandConfig,
    KeyboardCommandSource,
    TerminalKeyReader,
    format_keyboard_help,
)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--initial-cmd-vx", type=float, default=0.0)
    parser.add_argument("--initial-cmd-vy", type=float, default=0.0)
    parser.add_argument("--initial-cmd-yaw", type=float, default=0.0)
    parser.add_argument("--step-vx", type=float, default=0.1)
    parser.add_argument("--step-vy", type=float, default=0.1)
    parser.add_argument("--step-yaw", type=float, default=0.2)
    parser.add_argument("--max-vx", type=float, default=0.8)
    parser.add_argument("--max-vy", type=float, default=0.6)
...
def main(argv: list[str] | None = None) -> int:
    ...
    initial_command = np.array(
        [args.initial_cmd_vx, args.initial_cmd_vy, args.initial_cmd_yaw],
        dtype=np.float32,
    )

    with TerminalKeyReader() as reader:
        command_source = KeyboardCommandSource(
...
def run_keyboard_policy(
    ...
    logger.info(format_keyboard_help(key_config))
    ...
    logger.info(
        "keyboard MoE params: freq=%.1fHz action_clip=%.1f target_slew=%s "
        "warmup_steps=%d push_gains=%s kp=%.3f kv=%.3f torque_limits=%s",
        args.freq,
        args.max_action_abs,
    ...
    try:
        ...
        while not stop and not command_source.quit_requested:
    ...
    logger.info("keyboard MoE run done after %d iterations", iters)
    return 0
